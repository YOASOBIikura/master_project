...
from model import TDformer, ns_TDformer, DCTformer, DishTS, DishTS_Non, Dis_non
...
class Exp_Main(Exp_Basic):
    ...
    #     norm_model = norm_dict[self.args.norm].Model(self.args).float()
    #     model = Dis_non.Model(predict_model, norm_model, self.args).float()
    #     if self.args.use_multi_gpu and self.args.use_gpu:
    #         model = nn.DataParallel(model, device_ids=self.args.device_ids)
    #     return model
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            ...
                            outputs = self.model(None, None, batch_x, batch_x_mark, dec_inp, batch_y_mark)
                else:
                    if self.args.output_attention:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                    else:
