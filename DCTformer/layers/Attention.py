...
import os
from layers.dctnet import dct, dctc
...
class DCTAttention(nn.Module):
    def __init__(self, activation='softmax', output_attention=False):
    ...
    def forward(self, q, k, v, mask, tau, delta):
        ...
        xq_dct = self.transform(xq)
        xk_dct = self.transform(xk)
        xv_dct = self.transform(xv)

        xqk_dct = torch.einsum("bhex,bhry->bhxy", xq_dct, xk_dct) / sqrt(E)
        score = torch.softmax(xqk_dct.abs(), dim=-1)
        out = torch.einsum("bhxy,bhey->bhex", score, xv_dct)
        out = out.permute(0, 3, 1, 2)
        if self.output_attention == False:
            return (out, None)
        else:
...
DCT_Non Attention
"""
class DCT_NonAttention(nn.Module):
    def __init__(self, activation='softmax', output_attention=False):
        super(DCT_NonAttention, self).__init__()
        self.activation = activation
        self.output_attention = output_attention
    ...
    def forward(self, q, k, v, mask, tau=None, delta=None):
        ...
        xqk_dct = torch.einsum("bhex,bhry->bhxy", xq_dct, xk_dct) * tau + delta
        score = torch.softmax(scale * xqk_dct.abs(), dim=-1)
        out = torch.einsum("bhxy,bhey->bhex", score, xv_dct)
        out = out.permute(0, 3, 1, 2)
        if self.output_attention == False:
            return (out, None)
